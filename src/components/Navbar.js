            ...
            <div className="basis-1/2 flex md:basis-1/3 md:flex justify-end items-center mr-2">
                <div className="relative">
                    <div className={`absolute top-full right-0 bg-white ${showUserDropdown? 'px-4 py-2 sm:pt-6': 'p-0'} z-50 md:relative md:flex md: mr-4`}>
                        <div className={`${showUserDropdown? 'block' : 'hidden'} md:block cursor-pointer md:mr-4`} onClick={() => navigate('/shop-client/signup')}>REGISTER</div>
                        <div className={`${showUserDropdown? 'block' : 'hidden'} md:block cursor-pointer`} onClick={() => navigate('/shop-client/login')}>SIGN IN</div>
                    </div>
